...
This document demonstrates the **basic SMiXcan workflow** in four steps using
**small example data included in the package**.  
The example is intended to illustrate **how to run the functions**, not to
produce biological conclusions.
...
Cell-type fractions represent the proportion of each cell type contributing to
each bulk RNA-seq sample. These fractions can be estimated from marker genes.
...
- `markers`  
  Named list of marker genes defining each cell type.
  The list length determines the number of cell types K.
...
Next, we train genetic prediction models that allow SNP effects on expression to
...
SMiXcan tests gene–trait associations by combining trained SNP weights with GWAS
summary statistics while accounting for LD.
...
After running SMiXcan genome-wide, results can be post-processed using PRIMO to
classify cell-type association patterns among significant genes.
...
- `tab_specific_patterns`  
  Counts of association patterns among significant cell-type-specific genes.
