X_sampling <- function(X_pool, snp_region, N_sample){
  ...
  id = sample.int(ref_id_num, replace=TRUE, N_sample*2)
  X_sim <- X_pool[id[1:N_sample], snp_region]+
              X_pool[id[(N_sample+1):(2*N_sample)], snp_region]
  return(X_sim)
}
...
DataGen_newhap_binom <- function(mc=2, n.train, n.test, p, b0=0,
                                 ...
                                 var1, var2, seed=NULL, group = "heter xy",
                                 X_pool=X_pool, snp_region=snp_region){
  ...
  data=foreach (i = 1:mc)  %dorng% {
    ...
    beta1[1]=b0
    idx1=sample(2:(p+1), 2) # find two SNPs
    idx2=2*(rbinom(2, 1, 0.5)-0.5) # sign function
