...
# STEP1: load data--------
setwd('/Users/zhusinan/Downloads/S-MiXcan_code_folder/3pi_alpha08')
...
for(chr in 1:8){
  ...
  mw_gwas_input_path <- file.path(sprintf("baca2020_input/chr%d_mw_gwas_input_bcac2020_pi3_alpha08.rds", chr))
  mw_gwas_input <- readRDS(mw_gwas_input_path)

  # Load LD matrix, ref genome, SNP info
  LD_input_dir <- 'bcac2020_filtered_id/'
  LD_snp_path <- file.path(LD_input_dir,sprintf("filtered_chr%d_hg38_pi3_alpha08.bim", chr))
  X_ref_path <- file.path(LD_input_dir,sprintf("filtered_chr%d_hg38_012_pi3_alpha08.raw", chr))
  ...
  result_path <- file.path('c',sprintf("bcac2020_chr%d_result_pi3_alpha08.csv", chr))
  write.csv(real_result, result_path, row.names = FALSE)
}

combined <- do.call(rbind, lapply(1:6, function(chr) {
  result_path <- file.path("bcac2020_result",
                           sprintf("bcac2020_chr%d_result_pi3_alpha08.csv", chr))
...
plot(expected, observed,
     ...
     main="QQ Plot of SMiXcan_K in chr1-6",
