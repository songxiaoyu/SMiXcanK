# Prepare HERMES_HF HF GWAS for PWAS/S-MiXcan analysis.
#
# Inputs:
#   1. Heart protein weights from Analysis_code/Heart_Protein_Weights/2_train_heart_protein_weights.R
#   2. HERMES HF GWAS lifted from hg19 to hg38 by 1_liftover_hermes_hf_gwas.py
#
# Output:
...
gwas_file <- file.path(
  paper_dir,
  "Heart", "HERMES_HF",
  "FORMAT-METAL_Pheno1_EUR_hg38_rsid.tsv.gz"
)
...
filtered_id_dir <- file.path(workspace_dir, "hermes_hf_filtered_id")
...
# Step 2. Read lifted HERMES_HF GWAS once and standardize columns to the
...
if (length(missing_gwas_cols)) {
  stop("HERMES_HF GWAS is missing columns: ", paste(missing_gwas_cols, collapse = ", "))
}
if (!"rsid" %in% names(gwas)) {
  gwas[, rsid := NA_character_]
}

# HERMES_HF A1_beta is the beta for A1. Use A1 as Effect.Gwas and A2 as baseline.
gwas <- gwas %>%
  mutate(
...
for (chr in chr_list) {
  ...
  write.table(
    ...
    file = file.path(filtered_id_dir, sprintf("hermes_hf_filtered_chr%d_gwas_id_pwas.txt", chr)),
    col.names = FALSE,
    row.names = FALSE,
    quote = FALSE
  )
  ...
  saveRDS(
    mw_gwas_input,
    file = file.path(input_dir, sprintf("chr%d_mw_gwas_input_hermes_hf_pwas.rds", chr))
  )
}
