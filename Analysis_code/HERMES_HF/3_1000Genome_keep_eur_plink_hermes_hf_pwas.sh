...
GWAS_ID_DIR="${WORKSPACE_DIR}/hermes_hf_filtered_id"
...
for chr in ${CHR_LIST}; do
  ...
  "${PLINK2_BIN}" \
    ...
    --extract "${GWAS_ID_DIR}/hermes_hf_filtered_chr${chr}_gwas_id_pwas.txt" \
    --keep "${EUR_SAMPLES}" \
    --make-bed \
    --out "${GWAS_ID_DIR}/filtered_chr${chr}_hg38_hermes_hf_pwas"

  # Export additive 0/1/2 dosages. The filename keeps the historical "_012"
  # suffix because step 4 already reads this path.
  "${PLINK2_BIN}" \
    --bfile "${GWAS_ID_DIR}/filtered_chr${chr}_hg38_hermes_hf_pwas" \
    --export A \
    --out "${GWAS_ID_DIR}/filtered_chr${chr}_hg38_012_hermes_hf_pwas"
