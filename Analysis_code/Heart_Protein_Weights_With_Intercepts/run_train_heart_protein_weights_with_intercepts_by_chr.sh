...
run_one_chr() {
  ...
  local tissue_weights_file="${results_dir}/predixcan_tissue_weights_heart_protein${suffix}.csv"
  local model_terms_file="${results_dir}/model_terms_heart_protein_cardiomyocytes_other${suffix}.csv"

  if [ "${SKIP_EXISTING}" = "1" ] && [ -s "${weights_file}" ] && [ -s "${intercepts_file}" ] && [ -s "${tissue_weights_file}" ] && [ -s "${model_terms_file}" ]; then
  ...
  "${RSCRIPT_BIN}" Analysis_code/Heart_Protein_Weights_With_Intercepts/2_train_heart_protein_weights_with_intercepts.R \
