...
#' PRIMO MAP Classification for Multiple P-values
#'
...
#'
#' @param merged A data frame containing p-value columns used as PRIMO inputs.
#'   Each row is assumed to represent a cell-type-specific signal.
#'
#' @param pvals_names Character vector giving the column names of p-values
#'   to be used as input to PRIMO. All specified columns must exist in
#'   \code{merged}.
#'
#' @param alt_props Numeric scalar or numeric vector specifying the prior
#'   proportion of non-null tests for each p-value column. If a scalar is
#'   supplied, it is recycled to \code{length(pvals_names)}.
#'
#' @param prefix Character string used as a prefix for posterior probability
...
primo_map_all <- function(
  ...
  pmat <- as.matrix(merged[, pvals_names, drop = FALSE])
  storage.mode(pmat) <- "double"

  if (any(!is.finite(pmat))) {
    stop("Non-finite values (NA/Inf) in p-value matrix. Clean first.")
  }

  # ---- numerical safety (hard-coded eps) ----
  pmat <- pmin(pmax(pmat, 1e-15), 1 - 1e-15)

  # ---- run PRIMO ----
  res <- Primo::Primo_pval(pvals = pmat, alt_props = alt_props, ...)

  K <- ncol(res$post_prob)
  pat_names <- colnames(res$post_prob)
  if (is.null(pat_names) || length(pat_names) != K) {
    pat_names <- paste0("pat", seq_len(K))
  }

  post_mat <- as.matrix(res$post_prob)
  colnames(post_mat) <- paste0(prefix, pat_names)

  # ---- MAP classification (robust to NA) ----
  MAP_idx <- apply(post_mat, 1, function(x) {
    if (all(is.na(x))) return(NA_integer_)
    which.max(replace(x, is.na(x), -Inf))
  })
  MAP <- ifelse(is.na(MAP_idx), NA_character_, pat_names[MAP_idx])
